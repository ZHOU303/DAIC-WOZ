...
model.load_state_dict(torch.load(r"D:\DAIC-WOZ\weigth\best_visual.pth"))
...
# Predict and extract the output of the third residual block
output_dir = r"D:\DAIC-WOZ\parameter\visual"  
...
for idx in range(len(texts1)):
    ...
    sample_text3 = texts3[idx].unsqueeze(0).to(device)
    sample_text4 = texts4[idx].unsqueeze(0).to(device)
    label = labels1[idx].item()

    with torch.no_grad():
        output, layer3_output = model(sample_text1, sample_text2,sample_text3,sample_text4)
    ...
    if layer3_output_np.shape != (256, 28, 28):
        ...
        output_file = os.path.join(output_dir, f"{idx + 1}_visual.npy")
